...
class AJAXChat {
	...
	function insertMessage($text) {
		if(!$this->isAllowedToWriteMessage($this->getUserID()))
			return;
	...
	function getOnlineUsersData($channelIDs=null, $key=null, $value=null) {
		if($this->_onlineUsersData === null) {
			...
			$sql = 'SELECT
					...
					FROM
						'.$this->getDataBaseTable('online').'
					WHERE NOW() <= DATE_ADD(dateTime, interval '. $this->getConfig('inactiveTimeout') .' MINUTE)
					ORDER BY
						userName;';
	...
	function getGuestUser($userData) {
